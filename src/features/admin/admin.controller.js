...
exports.registerAdmin = catchAsync(async (req, res, next) => {
  ...
  // If user is authenticated, check if they're superadmin or admin
  if (req.user && req.user.role !== 'superadmin' && req.user.role !== 'admin') {
    return next(
      new AppError('Only superadmins or admins can register new admin users', 403)
    );
  }
...
exports.getFaceVerifications = catchAsync(async (req, res) => {
  const verifications = await User.find({
    ...
    .select(
      'userId name email verificationStatus uploadedPhoto createdAt updatedAt'
    )
    .sort('-createdAt');
...
exports.updateVerificationStatus = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOneAndUpdate(
    ...
    'userId name email verificationStatus verificationRemarks verifiedBy verifiedAt'
  );

  if (!user) {
    return next(
      new AppError('No verification request found with that ID', 404)
    );
  }
...
// (Duplicate block kept from your original – still same behavior)
...
// Admin User Management (older block using custom statics – kept as-is)
...
// ✅ Get all employees (MongoDB version)
exports.getAllEmployees = catchAsync(async (req, res, next) => {
  console.log('📊 Getting all employees from MongoDB...');
...
// Get all admin users (Mongo/Mongoose version – this one will override earlier)
...
exports.getAdvancedStats = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    !req.user.permissions.includes('system_management')
  ) {
  ...
  // Admin users stats
  const totalAdminUsers = await AdminUser.countDocuments();
  const activeAdminUsers = await AdminUser.countDocuments({
    status: { $ne: 'suspended' }
  });
  const suspendedAdminUsers = await AdminUser.countDocuments({
    status: 'suspended'
  });
    ...
    data: {
      systemStats: {
        ...
        organizers: {
          ...
          rejected:
            totalOrganizers - activeOrganizers - pendingOrganizers,
    ...
    data: {
      systemHealth: {
        ...
        services: {
          database: {
            status: dbStatus,
            responseTime: '12ms', // TODO: real ping
          ...
          storage: {
            status: 'healthy',
            freeSpace: '85%', // TODO: real disk check
            totalSpace: '500GB'
          }
        },
...
exports.bulkUserOperations = catchAsync(async (req, res, next) => {
  ...
  if (!req.user.permissions.includes('all_permissions')) {
    return next(
      new AppError('Only Super Admins can perform bulk operations', 403)
    );
  }
  ...
  if (!operation || !userIds || !Array.isArray(userIds)) {
    return next(
      new AppError('Operation and userIds array are required', 400)
    );
  }
      ...
      if (operation !== 'delete') {
        ...
        if (user) {
          ...
          results.push({
            ...
            status: 'success',
            message: `User ${
              operation + (operation.endsWith('e') ? 'd' : 'ed')
            } successfully`
          });
          successful++;
        } else {
...
exports.getUserActivity = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    const users = await User.find(filter)
      .select(
        'fullName email phone createdAt lastLogin status verificationStatus role'
      )
      .sort(sort)
      .skip(skip)
      .limit(parseInt(limit));
    ...
    const formattedUsers = users.map((user) => {
      ...
      if (user.lastLogin) {
        ...
        if (daysDiff === 0) {
          if (hoursDiff === 0) {
            lastLoginFormatted =
              minutesDiff === 0 ? 'Just now' : `${minutesDiff} minutes ago`;
            loginStatus = 'active';
          } else {
...
exports.checkFaceId = catchAsync(async (req, res, next) => {
  try {
    ...
    // TODO: implement actual deletion of face image records if required

    // Log the action for audit purposes
    console.log(
      `Admin ${req.user.userId} checked face data for user ${userId} at ${new Date().toISOString()}`
    );

    res.status(200).json({
      status: 'success',
      message:
        'User has face image data that requires manual removal from faceimage table',
      data: {
        user: {
          userId: user.userId,
          fullName: user.fullName,
